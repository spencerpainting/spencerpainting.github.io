...
ath.intl = {
	...
	en_us: {
		...
		android: 'To add this web app to the home screen tap <span class="ath-action-icon">icon</span> then <strong>Add to homescreen</strong>',
	},
...
ath.Class.prototype = {
	...
	show: function (force) {
		...
		if ( !this.applicationIcon ) {
			if ( ath.OS == 'ios' ) {
				this.applicationIcon = document.querySelector('head link[rel^=icon][sizes="192x192"],head link[rel^=icon][sizes="144x144"],head link[rel^=icon][sizes="120x120"],head link[rel^=icon][sizes="114x114"],head link[rel^=icon]');
			} else {
				this.applicationIcon = document.querySelector('head link[rel^="shortcut icon"][sizes="196x196"],head link[rel^=icon]');
			}
		}
